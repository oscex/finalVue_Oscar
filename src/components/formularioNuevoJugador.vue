<template>
  <form class="container">
    ...
    <select name="equipo" id="equipo" v-model="equipoSeleccionado" :disabled="equipoFijo != null">
      <option v-for="equipo in equipos" :value="equipo" :key="equipo.name">{{ equipo.name }}</option>
    </select>

    <label for="goles">Número de goles metidos</label>
...
input, select, button {
    ...
    border-radius: 5px;
    font-size: 16px;
    width: 100%; /* Hacer que los inputs y select ocupen todo el ancho */
}
